module.exports = function (conn) {
  ...
  router.post('/submit', (req, res) => {
    ...
    let sqlStr = `INSERT INTO genshin (data, time) VALUES('${data}', '${time}')`;
  ...
  router.post('/search', (req, res) => {
    ...
    if (data === "" && time === "") {
      sqlStr = `SELECT * FROM genshin`;
    } else if (data === "") {
      time = moment(time).tz('Asia/Shanghai').format();
      time = time.substring(0, 10); // "2024-03-11"
      console.log("修改的数据", data, time);
      sqlStr = `SELECT * FROM genshin where time = '${time}'`; // 使用单个等号进行比较
    } else if (time === "") {
      sqlStr = `SELECT * FROM genshin where data LIKE '%${data}%'`; // 使用单个等号进行比较
    } else {
      time = moment(time).tz('Asia/Shanghai').format();
      time = time.substring(0, 10); // "2024-03-11"
      console.log("修改的数据", data, time);
      sqlStr = `SELECT * FROM genshin WHERE time = '${time}' AND data LIKE '%${data}%'`;
    }
